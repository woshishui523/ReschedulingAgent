function [timetableAfter,OP]=LinearOptimizationChange(Tnum,Pnum,delay,timetable,varargin)
%{
修改部分： 
1.晚点设计
2.输出设计
...
%                 if j==4
%                     if i==1
%                         C = [C,depart_time(j,i)-arrive_time(j-1,i)==firstTimetable(j,i)-firstTimetable(j-1,i)];
%                     else
%                         C = [C,depart_time(j,i)-arrive_time(j-1,i)==firstPM(j,i)*TsJ+(depart_time(j,i)-depart_time(j,i-1))*P*ckspeed];
%                     end
% 
%                 else
%                     if i==1
%                         if j>8
%                             C = [C,depart_time(j,i)-arrive_time(j-1,i)==firstPM(j,i)*Ts+(depart_time(j,i)-depart_time(j,i-1))*P*ckspeed];
%                         else
%                             C = [C,depart_time(j,i)-arrive_time(j-1,i)==firstTimetable(j,i)-firstTimetable(j-1,i)];
%                         end
%                         
...
for i=1:(sec_num-1)
    for j=1:train_num
        for k=1:train_num
            if k<j
                C = [C,depart_time(i+1,k)-arrive_time(i,j)<=-Tz];
            end
        end
    end
end
...
for i = 1:sec_num
    for j = 1:train_num
        for k = 1:train_num
            if j>k
                C = [C,depart_time(i,j)-depart_time(i,k)>=Tz];
                C = [C,arrive_time(i,j)-arrive_time(i,k)>=Tz];
            end
        end
    end
end
...
% end
% for i=2:size(depart_time,2)%列车数
%    for  j=1:size(depart_time,1)%站台数
%        DelayFa=DelayFa+abs(depart_time(j,i)-depart_time(j,i-1)-H);
%    end
% end
% for i=1:size(depart_time,2)%列车数
%    for  j=1:size(depart_time,1)-1%站台数
%        DelayFa=DelayFa+abs((depart_time(j+1,i)-depart_time(j,i))-(firstTimetable(2*(j+1),i)-firstTimetable(2*j,i)));
%    end
% end

DelayFa = sum(sum(depart_time-firstTimetable(4:2:end,:))); % 发车时间
